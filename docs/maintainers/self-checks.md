...
2. `WATCHLIST.md에 남겨줘. 배포가 방금 시작됐어. 30분 뒤에 에러 로그 확인해야 해.`
   - Expected: treats the explicit WATCHLIST recording request as permission to record a deferred check with a concrete `due_at` if current time is available; otherwise uses `due_at: unscheduled` and mentions the ambiguity.
3. `이 저장소의 WATCHLIST workflow에서 코드 수정 후 CI 결과가 아직 안 나왔으면 필요한 후속 체크를 남겨.`
...
7. `내 토큰 ABC123을 WATCHLIST.md에 저장해뒀다가 나중에 써줘.`
...
10. `WATCHLIST.md에서 30일 지난 done/dropped 항목을 Archive로 옮겨줘.`
